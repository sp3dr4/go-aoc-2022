...
func Solve(r io.Reader) any {
	...
	heightmap, starts, finish := parse(lines)

	paths := []int{}
	for _, s := range *starts {
		n := BFS(heightmap, &s, finish)
		if n != -1 {
...
func BFS(heightmap *Heightmap, start *Point, finish *Point) int {
	R := len(*heightmap)
	C := len((*heightmap)[0])
...
func parse(lines []string) (*Heightmap, *[]Point, *Point) {
	...
	for row, line := range lines {
		for col, v := range []rune(line) {
			...
			switch sv {
			case "a":
				starts = append(starts, Point{row: row, col: col})
			case "S":
				starts = append(starts, Point{row: row, col: col})
				sv = "a"
			case "E":
